...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $petugas            = $_SESSION['id'];
    $status             = 1; // 1 = Pending, 2 = Valid, 3 = Tidak Valid
    ...
    if (isset($_FILES['fileupload'])) {
        if ($ukuran_file > $max_file_size) {
            ...
            $nama_file_baru = 'surat-masuk/' . $tanggal . '-' . uniqid() . '.' . $ext_file;

            $path = "../../assets/file/" . $nama_file_baru;
            if ($con->cek_nomor_surat($nomor)) {
                $pesanNomor = "Nomor surat sudah digunakan!";
            } else {
                $data = $con->tambah_surat($kategori, $petugas, $instansi, $nama, $nomor, $tanggal, $deskripsi, $nama_file_baru, $status, $tipe_surat);
